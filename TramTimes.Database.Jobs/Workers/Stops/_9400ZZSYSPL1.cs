...
public class _9400ZZSYSPL1(
    BlobServiceClient blobService,
    NpgsqlDataSource dataSource,
    ILogger<_9400ZZSYSPL1> logger) : IJob {
    ...
    public async Task Execute(IJobExecutionContext context)
    {
        ...
        try
        {
            ...
            switch (context.FireTimeUtc.Date.DayOfWeek)
            {
                case DayOfWeek.Monday:
                {
                    if (databaseResults.Count != activeSchedule?.Monday?.Count)
                        logger.LogWarning(message: "9400ZZSYSPL1: Database service count does not match schedule");
            ...
            switch (context.FireTimeUtc.Date.DayOfWeek)
            {
                case DayOfWeek.Monday:
                {
                    if (storageResults.Count != activeSchedule?.Monday?.Count)
                        logger.LogWarning(message: "9400ZZSYSPL1: Storage service count does not match schedule");
            ...
            switch (context.FireTimeUtc.Date.DayOfWeek)
            {
                ...
                default:
                    logger.LogWarning(message: "9400ZZSYSPL1: Service not found in schedule");
