...
public class _9400ZZSYMWY1(
    BlobServiceClient blobService,
    NpgsqlDataSource dataSource,
    ILogger<_9400ZZSYMWY1> logger) : IJob {
    ...
    public async Task Execute(IJobExecutionContext context)
    {
        ...
        try
        {
            ...
            switch (context.FireTimeUtc.Date.DayOfWeek)
            {
                case DayOfWeek.Monday:
                {
                    if (databaseResults.Count != activeSchedule?.Monday?.Count)
                        logger.LogWarning(message: "9400ZZSYMWY1: Database service count does not match schedule");
            ...
            switch (context.FireTimeUtc.Date.DayOfWeek)
            {
                case DayOfWeek.Monday:
                {
                    if (storageResults.Count != activeSchedule?.Monday?.Count)
                        logger.LogWarning(message: "9400ZZSYMWY1: Storage service count does not match schedule");
            ...
            switch (context.FireTimeUtc.Date.DayOfWeek)
            {
                ...
                default:
                    logger.LogWarning(message: "9400ZZSYMWY1: Service not found in schedule");
