...
public class _9400ZZSYGRC2(
    BlobServiceClient blobService,
    NpgsqlDataSource dataSource,
    ILogger<_9400ZZSYGRC2> logger) : IJob {
    ...
    public async Task Execute(IJobExecutionContext context)
    {
        ...
        try
        {
            ...
            switch (context.FireTimeUtc.Date.DayOfWeek)
            {
                case DayOfWeek.Monday:
                {
                    if (databaseResults.Count != activeSchedule?.Monday?.Count)
                        logger.LogWarning(message: "9400ZZSYGRC2: Database service count does not match schedule");
            ...
            switch (context.FireTimeUtc.Date.DayOfWeek)
            {
                case DayOfWeek.Monday:
                {
                    if (storageResults.Count != activeSchedule?.Monday?.Count)
                        logger.LogWarning(message: "9400ZZSYGRC2: Storage service count does not match schedule");
            ...
            switch (context.FireTimeUtc.Date.DayOfWeek)
            {
                ...
                default:
                    logger.LogWarning(message: "9400ZZSYGRC2: Service not found in schedule");
