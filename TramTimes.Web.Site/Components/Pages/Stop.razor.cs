...
public partial class Stop : ComponentBase, IAsyncDisposable
{
    ...
    protected override async Task OnParametersSetAsync()
    {
        ...
        if (NavigationService.Uri.EqualsIgnoreCase(value: NavigationService.BaseUri + $"stop/{StopId}"))
        {
    ...
    private async Task OnLayoutChangedAsync()
    {
        ...
        if (JavascriptManager is not null)
            await JavascriptManager.InvokeVoidAsync(
                identifier: "writeConsole",
                args: $"stop: layout changed {LayoutService.Layout}");
    ...
    private async Task OnListReadAsync(ListViewReadEventArgs readEventArgs)
    {
        ...
        try
        {
            ...
            data = await CacheService.GetServicesByStopAsync(
                id: NextStop.Id ?? StopId,
        ...
        if (data is null)
        {
            try
            {
                ...
                data = await DatabaseService.GetServicesByStopAsync(
                    id: NextStop.Id ?? StopId,
                    cancellationToken: ListSource.Token);
    ...
    private async Task OnListChangeAsync(
        ...
        if (NavigationService.Uri.ContainsIgnoreCase(value: $"/trip/{tripId}/{stopId}"))
            return;
