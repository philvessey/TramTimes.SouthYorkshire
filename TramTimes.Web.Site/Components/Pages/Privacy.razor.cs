...
public partial class Privacy : ComponentBase, IAsyncDisposable
{
    ...
    protected override async Task OnParametersSetAsync()
    {
        ...
        if (NavigationService.Uri.EqualsIgnoreCase(value: NavigationService.BaseUri + "privacy"))
        {
    ...
    private async Task OnConsentChangedAsync()
    {
        ...
        if (JavascriptManager is not null)
            await JavascriptManager.InvokeVoidAsync(
                identifier: "writeConsole",
                args: $"privacy: consent changed {(ConsentService.Consent is true ? "accepted" : "rejected")}");
    ...
    private async Task OnLayoutChangedAsync()
    {
        ...
        if (JavascriptManager is not null)
            await JavascriptManager.InvokeVoidAsync(
                identifier: "writeConsole",
                args: $"privacy: layout changed {LayoutService.Layout}");
    ...
    private async Task OnSchemeChangedAsync()
    {
        ...
        if (JavascriptManager is not null)
            await JavascriptManager.InvokeVoidAsync(
                identifier: "writeConsole",
                args: $"privacy: scheme changed {SchemeService.Scheme}");
