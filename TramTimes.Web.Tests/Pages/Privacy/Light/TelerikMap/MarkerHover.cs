...
public class MarkerHover(AspireManager aspireManager) : BaseTest(aspireManager: aspireManager)
{
    ...
    [Theory]
    [InlineData("Halfway", 53.32853446547, -1.344313639, 1)]
    [InlineData("Malin Bridge", 53.40064755511, -1.50821200817, 2)]
    [InlineData("Middlewood", 53.41586395553, -1.51006771516, 3)]
        ...
        await RunTestAsync(cookies: ConsentCookies.Accepted, scheme: ColorScheme.Light, test: async page =>
        {
            ...
            await File.WriteAllBytesAsync(
                path: Path.Combine(
                    ...
                    path2: $"privacy|light|telerik-map|marker-hover|run{run}|desktop.png"),
                bytes: Screenshot ?? []);
    ...
    public async Task Mobile(
        ...
        await RunTestAsync(cookies: ConsentCookies.Rejected, scheme: ColorScheme.Light, test: async page =>
        {
            ...
            await File.WriteAllBytesAsync(
                path: Path.Combine(
                    path1: AspireManager.Storage!.FullName,
                    path2: $"privacy|light|telerik-map|marker-hover|run{run}|mobile.png"),
                bytes: Screenshot ?? []);
