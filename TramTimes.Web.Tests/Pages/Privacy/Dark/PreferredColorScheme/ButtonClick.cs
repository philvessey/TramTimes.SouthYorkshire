...
namespace TramTimes.Web.Tests.Pages.Privacy.Dark.PreferredColorScheme;
        ...
        await RunTestAsync(cookies: ConsentCookies.Accepted, scheme: ColorScheme.Dark, test: async page =>
        {
            ...
            await File.WriteAllBytesAsync(
                path: Path.Combine(
                    ...
                    path2: $"privacy|dark|preferred-color-scheme|button-click|run{run}|desktop.png"),
                bytes: Screenshot ?? []);
    ...
    public async Task Mobile(
        ...
        await RunTestAsync(cookies: ConsentCookies.Accepted, scheme: ColorScheme.Dark, test: async page =>
        {
            ...
            await File.WriteAllBytesAsync(
                path: Path.Combine(
                    path1: AspireManager.Storage!.FullName,
                    path2: $"privacy|dark|preferred-color-scheme|button-click|run{run}|mobile.png"),
                bytes: Screenshot ?? []);
