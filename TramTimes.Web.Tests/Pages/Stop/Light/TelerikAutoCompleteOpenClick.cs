...
namespace TramTimes.Web.Tests.Pages.Stop.Light;

public class TelerikAutoCompleteOpenClick(AspireManager aspireManager) : BaseTest(aspireManager: aspireManager)
{
    ...
    public async Task ExtraSmall(
        ...
        await RunTestLightModeAsync(test: async page =>
        {
            ...
            try
            {
                ...
                await Assertions
                    .Expect(page: page)
                    .ToHaveURLAsync(urlOrRegExp: new Regex(
                        pattern: id,
                        options: RegexOptions.None));
                
                await Assertions
                    .Expect(page: page)
                    .ToHaveTitleAsync(titleOrRegExp: new Regex(
                        pattern: name,
                        options: RegexOptions.None));
            ...
            await File.WriteAllBytesAsync(
                path: Path.Combine(
                    ...
                    path2: $"stop|light|telerik-auto-complete-open-click|run{run}|01.png"),
                bytes: Screenshot ?? []);
    ...
    public async Task Small(
        ...
        await RunTestLightModeAsync(test: async page =>
        {
            ...
            await File.WriteAllBytesAsync(
                path: Path.Combine(
                    path1: AspireManager.Storage!.FullName,
                    path2: $"stop|light|telerik-auto-complete-open-click|run{run}|02.png"),
                bytes: Screenshot ?? []);
    ...
    public async Task Medium(
        ...
        await RunTestLightModeAsync(test: async page =>
        {
            ...
            await File.WriteAllBytesAsync(
                path: Path.Combine(
                    path1: AspireManager.Storage!.FullName,
                    path2: $"stop|light|telerik-auto-complete-open-click|run{run}|03.png"),
                bytes: Screenshot ?? []);
    ...
    public async Task Large(
        ...
        await RunTestLightModeAsync(test: async page =>
        {
            ...
            await File.WriteAllBytesAsync(
                path: Path.Combine(
                    path1: AspireManager.Storage!.FullName,
                    path2: $"stop|light|telerik-auto-complete-open-click|run{run}|04.png"),
                bytes: Screenshot ?? []);
    ...
    public async Task ExtraLarge(
        ...
        await RunTestLightModeAsync(test: async page =>
        {
            ...
            await File.WriteAllBytesAsync(
                path: Path.Combine(
                    path1: AspireManager.Storage!.FullName,
                    path2: $"stop|light|telerik-auto-complete-open-click|run{run}|05.png"),
                bytes: Screenshot ?? []);
    ...
    public async Task ExtraExtraLarge(
        ...
        await RunTestLightModeAsync(test: async page =>
        {
            ...
            await File.WriteAllBytesAsync(
                path: Path.Combine(
                    path1: AspireManager.Storage!.FullName,
                    path2: $"stop|light|telerik-auto-complete-open-click|run{run}|06.png"),
                bytes: Screenshot ?? []);
