...
namespace TramTimes.Web.Tests.Pages.Stop.Light.TelerikAutoComplete;
        ...
        await RunTestAsync(cookie: ConsentCookies.False, scheme: ColorScheme.Light, test: async page =>
        {
            ...
            await File.WriteAllBytesAsync(
                path: Path.Combine(
                    ...
                    path2: $"stop|light|telerik-auto-complete|close|run{run}|01.png"),
                bytes: Screenshot ?? []);
    ...
    public async Task Small(
        ...
        await RunTestAsync(cookie: ConsentCookies.False, scheme: ColorScheme.Light, test: async page =>
        {
            ...
            await File.WriteAllBytesAsync(
                path: Path.Combine(
                    path1: AspireManager.Storage!.FullName,
                    path2: $"stop|light|telerik-auto-complete|close|run{run}|02.png"),
                bytes: Screenshot ?? []);
    ...
    public async Task Medium(
        ...
        await RunTestAsync(cookie: ConsentCookies.False, scheme: ColorScheme.Light, test: async page =>
        {
            ...
            await File.WriteAllBytesAsync(
                path: Path.Combine(
                    path1: AspireManager.Storage!.FullName,
                    path2: $"stop|light|telerik-auto-complete|close|run{run}|03.png"),
                bytes: Screenshot ?? []);
    ...
    public async Task Large(
        ...
        await RunTestAsync(cookie: ConsentCookies.False, scheme: ColorScheme.Light, test: async page =>
        {
            ...
            await File.WriteAllBytesAsync(
                path: Path.Combine(
                    path1: AspireManager.Storage!.FullName,
                    path2: $"stop|light|telerik-auto-complete|close|run{run}|04.png"),
                bytes: Screenshot ?? []);
    ...
    public async Task ExtraLarge(
        ...
        await RunTestAsync(cookie: ConsentCookies.False, scheme: ColorScheme.Light, test: async page =>
        {
            ...
            await File.WriteAllBytesAsync(
                path: Path.Combine(
                    path1: AspireManager.Storage!.FullName,
                    path2: $"stop|light|telerik-auto-complete|close|run{run}|05.png"),
                bytes: Screenshot ?? []);
    ...
    public async Task ExtraExtraLarge(
        ...
        await RunTestAsync(cookie: ConsentCookies.False, scheme: ColorScheme.Light, test: async page =>
        {
            ...
            await File.WriteAllBytesAsync(
                path: Path.Combine(
                    path1: AspireManager.Storage!.FullName,
                    path2: $"stop|light|telerik-auto-complete|close|run{run}|06.png"),
                bytes: Screenshot ?? []);
