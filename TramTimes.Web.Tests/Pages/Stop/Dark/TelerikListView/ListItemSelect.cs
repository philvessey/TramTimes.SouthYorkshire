...
namespace TramTimes.Web.Tests.Pages.Stop.Dark.TelerikListView;
    ...
    public async Task Desktop(
        ...
        var tripId = points
            .Where(predicate: point => point.DepartureDateTime >= new DateTime(
                ...
                second: 0))
            .ElementAtOrDefault(index: 2)?.TripId ?? string.Empty;
        ...
        await RunTestAsync(cookies: ConsentCookies.Accepted, scheme: ColorScheme.Dark, test: async page =>
        {
            ...
            try
            {
                ...
                await Assertions
                    .Expect(page: page)
                    .ToHaveURLAsync(urlOrRegExp: new Regex(pattern: $"/trip/{tripId}/{stopId}"));
            ...
            await File.WriteAllBytesAsync(
                path: Path.Combine(
                    ...
                    path2: $"stop|dark|telerik-list-view|list-item-select|run{run}|desktop.png"),
                bytes: Screenshot ?? []);
    ...
    public async Task Mobile(
        ...
        await RunTestAsync(cookies: ConsentCookies.Rejected, scheme: ColorScheme.Dark, test: async page =>
        {
            ...
            await File.WriteAllBytesAsync(
                path: Path.Combine(
                    path1: AspireManager.Storage!.FullName,
                    path2: $"stop|dark|telerik-list-view|list-item-select|run{run}|mobile.png"),
                bytes: Screenshot ?? []);
