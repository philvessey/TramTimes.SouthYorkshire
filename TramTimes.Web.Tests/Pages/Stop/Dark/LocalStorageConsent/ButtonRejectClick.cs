        ...
        await RunTestAsync(cookie: ConsentCookies.Unknown, scheme: ColorScheme.Dark, test: async page =>
        {
            ...
            await File.WriteAllBytesAsync(
                path: Path.Combine(
                    ...
                    path2: $"stop|dark|local-storage-consent|button-reject-click|run{run}|desktop.png"),
                bytes: Screenshot ?? []);
    ...
    public async Task Mobile(
        ...
        await RunTestAsync(cookie: ConsentCookies.Unknown, scheme: ColorScheme.Dark, test: async page =>
        {
            ...
            await File.WriteAllBytesAsync(
                path: Path.Combine(
                    path1: AspireManager.Storage!.FullName,
                    path2: $"stop|dark|local-storage-consent|button-reject-click|run{run}|mobile.png"),
                bytes: Screenshot ?? []);
