...
public class ComboBoxSelect(AspireManager aspireManager) : BaseTest(aspireManager: aspireManager)
{
    ...
    [Theory]
    [InlineData("9400ZZSYHFW1", "Halfway", 53.32853446547, -1.344313639, "half", 1)]
    [InlineData("9400ZZSYMAL1", "Malin Bridge", 53.40064755511, -1.50821200817, "mali", 2)]
    [InlineData("9400ZZSYMID1", "Middlewood", 53.41586395553, -1.51006771516, "midd", 3)]
        ...
        await RunTestAsync(cookies: ConsentCookies.Accepted, scheme: ColorScheme.Light, test: async page =>
        {
            ...
            await File.WriteAllBytesAsync(
                path: Path.Combine(
                    ...
                    path2: $"home|light|telerik-auto-complete|combo-box-select|run{run}|desktop.png"),
                bytes: Screenshot ?? []);
    ...
    public async Task Mobile(
        ...
        await RunTestAsync(cookies: ConsentCookies.Accepted, scheme: ColorScheme.Light, test: async page =>
        {
            ...
            await File.WriteAllBytesAsync(
                path: Path.Combine(
                    path1: AspireManager.Storage!.FullName,
                    path2: $"home|light|telerik-auto-complete|combo-box-select|run{run}|mobile.png"),
                bytes: Screenshot ?? []);
