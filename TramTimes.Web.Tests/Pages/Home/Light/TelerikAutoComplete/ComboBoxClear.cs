...
public class ComboBoxClear(AspireManager aspireManager) : BaseTest(aspireManager: aspireManager)
{
    ...
    [Theory]
    [InlineData(53.32853446547, -1.344313639, "halfw", 1)]
    [InlineData(53.40064755511, -1.50821200817, "malin", 2)]
    [InlineData(53.41586395553, -1.51006771516, "middl", 3)]
        ...
        await RunTestAsync(cookies: ConsentCookies.Accepted, scheme: ColorScheme.Light, test: async page =>
        {
            ...
            await File.WriteAllBytesAsync(
                path: Path.Combine(
                    ...
                    path2: $"home|light|telerik-auto-complete|combo-box-clear|run{run}|desktop.png"),
                bytes: Screenshot ?? []);
    ...
    public async Task Mobile(
        ...
        await RunTestAsync(cookies: ConsentCookies.Accepted, scheme: ColorScheme.Light, test: async page =>
        {
            ...
            await File.WriteAllBytesAsync(
                path: Path.Combine(
                    path1: AspireManager.Storage!.FullName,
                    path2: $"home|light|telerik-auto-complete|combo-box-clear|run{run}|mobile.png"),
                bytes: Screenshot ?? []);
