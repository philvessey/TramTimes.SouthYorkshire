...
namespace TramTimes.Web.Tests.Pages.Home.Dark.BannerVertical;
        ...
        await RunTestAsync(cookie: ConsentCookies.True, scheme: ColorScheme.Dark, test: async page =>
        {
            ...
            await File.WriteAllBytesAsync(
                path: Path.Combine(
                    ...
                    path2: $"home|dark|banner-vertical|placeholder-visible|run{run}|desktop.png"),
                bytes: Screenshot ?? []);
    ...
    public async Task Mobile(
        ...
        await RunTestAsync(cookie: ConsentCookies.False, scheme: ColorScheme.Dark, test: async page =>
        {
            ...
            await File.WriteAllBytesAsync(
                path: Path.Combine(
                    path1: AspireManager.Storage!.FullName,
                    path2: $"home|dark|banner-vertical|placeholder-visible|run{run}|mobile.png"),
                bytes: Screenshot ?? []);
